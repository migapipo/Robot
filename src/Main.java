...
/**
 *
 * The Main program implements a CLI application to parse commands and display the result.
 * <p>
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String welcomeMessage = ("Welcome to the Robot Application! Here is the main menu. Please select your choice " +
                "with a number: \n" + "1. Create a new robot to start the game \n" + "2. Exit the application\n" +
        ...
        try {
            ...
            do {
                ...
                switch (userChoice) {
                    case 1:
                        // Initialise a robot object with starting point as (0, 0), starting direction as N North, distance
                        // from start point as 0
                        Robot robot = new Robot(0, 0, 0, 0);
                        System.out.println("Your robot has been created successfully! Now try to write " +
                                "some commands to make it move! ");

                        givenCommandString = scanner.nextLine();

                        System.out.println("Your robot will move according to the following commands " +
                                givenCommandString);
                        ...
                        // Parse the given command String into the ArrayList
                        ArrayList<String> givenCommandList = parseGivenCommandStringIntoList(givenCommandString);

                        // Move robot by the given commands one by one
                        moveRobotByCommand(robot, givenCommandList);

//                        System.out.printf("Your robot's final coordinate is (%s,%s) \n", robot.getCoordinateX(), robot
//                                .getCoordinateY());
//
//                        System.out.printf("Your robot's final facing direction is %s \n", facingDirection(robot
//                                .getFacingDirection()));
                    ...
                    case 2:
                        // If userInput is 2, exit the application
                        exit = true;
    ...
    /**
     * Move the robot according to the given commands in the command list, there are 4 main available commands:
     ...
     * @param givenCommandList the command list given by the user
     */
        ...
        for (int i = 0; i < givenCommandList.size(); i++) {
            ...
            switch (commandType) {
                ...
                case "L":
                    ...
                    break;
                // If the command is either "F" or "B", change x or y coordinate according to the current facing
                // direction
                case "F":
